...
${PASSWORD}      admin12345
${NAME}          Bakso
${DESCRIPTION}   Ini adalah Bakso
${IMAGE_PATH}    E:/Download/1.png  # Corrected path to the image file
${PRICE}         limabelasribu
...
LoginTest
    ...
    Click Element    xpath=//a[@data-target='#Product']
    Sleep    1    # Give time for the menu to expand
    Click Element    xpath=//a[contains(@class, 'collapse-item') and contains(@href, '/admin/product/create')]
    Sleep    2
    Input Text    id=name    ${NAME}
    Input Text    id=description    ${DESCRIPTION}
    Choose File   id=image    ${IMAGE_PATH}
    Input Text    id=price    ${PRICE}
    Select From List By Value    id=category_id    1  # Selecting category by value
